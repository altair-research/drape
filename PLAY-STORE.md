...
> 갱신: 2026-09-22 11:30
> 상태: 비공개 테스트(Alpha) 심사 제출 완료. 스토어 등록정보·앱 콘텐츠 전부 입력됨.
> 다음: 심사 승인 대기(며칠) → 테스터 12명 채우기 → 14일 연속 테스트 → 프로덕션 액세스 신청
> 막힘: 테스터 12명 미모집 (안드로이드 Gmail 주소 필요)
> 더 볼 곳: 진행 상황의 '남은 순서' 표, §7-3 비공개 테스트 기록
...
| 4 | **심사 승인 대기** | 대기 | 며칠 |
| 5 | **테스터 12명 모으기** — 안드로이드 쓰는 지인 Gmail | 사용자 | — |
| 6 | **14일 연속 테스트** — 테스터가 실제로 써야 한다 | 대기 | **14일** |
| 7 | 프로덕션 액세스 신청 → 승인 | 사용자 + 대기 | 며칠 |
| 8 | 프로덕션 출시 → 심사 | 대기 | 며칠 |
| 9 | 출시 후: 조직 프로필에 스토어 링크, 한국어 번역 추가 | 같이 | 20분 |


**병목은 6번이다.** 14일 시계는 심사 승인 후 테스터가 실제로 설치·사용해야 돌기 시작한다.
