...
> 갱신: 2026-09-18 21:20
> 상태: 내부 테스트 설치 성공. 주소창 없이 전체화면으로 열림(TWA 검증 통과).
> 다음: 스크린샷 4장 → 스토어 등록정보 입력 → 앱 콘텐츠 답변 → 비공개 테스트(12명×14일)
> 막힘: 없음
> 더 볼 곳: §4-1 서명 키 함정, §6-0 콘솔 입력값, §6-2b 문안, §7-2 업로드 순서
...
- [x] **폰에서 주소창 없이 전체화면으로 열림 확인 (09-18)** — TWA 검증 통과
- [x] 내부 테스트 설치 성공 (opt-in 링크 → Play 설치)
- [ ] 두 번째 계정으로도 설치 확인
- [ ] 폰 스크린샷 2~8장 (지금이 적기 — 주소창 없음)
- [ ] 스토어 등록정보(§6-2b 문안 + §6-3 이미지) 입력
- [ ] 앱 콘텐츠 답변(§6-0 B 표)
