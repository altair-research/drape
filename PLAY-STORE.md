...
> 갱신: 2026-09-19 21:40
> 상태: 앱은 내부 테스트로 정상 설치·전체화면 동작. 스토어 자료(문안·이미지·스크린샷) 전부 준비됨.
> 다음: 스토어 등록정보 입력 → 앱 콘텐츠 답변 → 비공개 테스트 12명 개설(14일 시계 시작)
> 막힘: 없음 (사용자 차례 — 콘솔 입력)
> 더 볼 곳: 진행 상황의 '남은 순서' 표, §6-0 콘솔 입력값, §6-2b 문안, §4-1 서명 키 함정
...
| # | 할 일 | 누가 | 걸리는 시간 |
|---|---|---|---|
| 1 | **스토어 등록정보** — 문안(§6-2b) + 이미지 3종(§6-3) | 사용자 | 20분 |
| 2 | **앱 콘텐츠 답변** — §6-0 B 표대로 | 사용자 | 20분 |
| 3 | 두 번째 계정으로 설치 확인 (선택) | 사용자 | 5분 |
| 4 | **비공개 테스트 트랙 개설 + 테스터 12명 등록** | 사용자 | 30분 |
| 5 | **14일 대기** — 테스터가 실제로 써야 한다 | 대기 | **14일** |
| 6 | 프로덕션 액세스 신청 → 승인 | 사용자 + 대기 | 며칠 |
| 7 | 프로덕션 출시 → 심사 | 대기 | 며칠 |
| 8 | 출시 후: 조직 프로필에 스토어 링크, 한국어 번역 추가 | 사용자 + Claude | 20분 |

**병목은 5번이다.** 14일 시계는 비공개 테스트를 시작해야 돌기 시작하므로, 1~4번을 빨리 끝내는 것이 전체 일정을 줄인다.
